...
| **declared extents rot; measured ones do not** | **8** | 3 predecessor films cropped their own payoff; 3 of 5 hand-computed extents wrong on the 2026-07-25 film `(local)`; two more on the 2026-08-07 plugin-only builds `(local)`, found by probe before a frame was rendered | `subjectFromObject` — Track D, promoted. The predecessor *specified* the fix and it never shipped, while a code comment claimed it had |
