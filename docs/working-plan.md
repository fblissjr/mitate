...
> **The restructure migration is over.** `restructure-2026-07.md` completed and
> deleted itself 2026-08-04 on R5's closure, per its own retirement rule; its
> permanent record is the CHANGELOG entries (0.16.30–0.16.67); the closure
> record was archived out of the tree 2026-08-07 (owner call — snapshots are
> no longer tracked). **The live queue is [`README.md`](README.md)'s
> work-next row**; this file is the standing backlog behind it.
>
> **Three wholly-consumed sections moved out the same day**, under the
> `when-absent` marker that fired on the deletion: the sequencing table
> (superseded as a queue, status column frozen at 2026-07-30), A1 (shipped as
> `build.js probe`), and the 2026-07-31 BLOCKING MERGE review (all 15 findings
> closed, PR #3 merged). The superseded sections were preserved verbatim in
> the 2026-08-04 snapshot, archived out of the tree 2026-08-07. What was still true in the
> sequencing table survives here instead of dying with it: **Track A items 2
> (`build.js transitions`) and 3 (cost reporting) remain unbuilt**, and item
> 4's limit-wins-tiebreaker half still waits on item 2. Sections describing
> executed work stayed where they still home something live — A0's
> pre-registered reviewer-prescription control and its answer key, Track D's
> unshipped `STYLE.palette` and `CONFIG.name`/`titleCard` split.
...
(the restructure plan's homes table, which also named `internal/log/`, retired
with it 2026-08-04) · `grep -rn "internal/log"` over tracked
...
### The declared-substitution rule → the 0.18.2 cascade (landed 2026-08-04)
...
| `tableValue` in `build.js` | a mini-reader of JS declaration syntax (slicer, comment-skipper, mutation scanner) | **already fired** — this row said "a third false-verdict finding" and a same-day audit corrected the count: the record holds at least four (loop-built silent green, 0.16.68; the dur-const false ERROR and the unparseable silent green, both 0.16.70; then the fraction and dot-assignment findings above). What blocks conversion is design, not evidence: "execute the subject" has no obvious meaning for a static source reader — the candidate shapes (evaluate the scene's declaration prefix in a sandbox; widen the contract to export the tables and give up the static premise) trade away things `check`'s header currently promises. **DISPOSITIONED 2026-08-04 (owner): the reader stays; both conversions declined against the record.** Sandbox evaluation violates the representation decision's point 1 — data is what is decidable *without running the scene*, so executing the film to validate its data inverts the boundary, and the stub environment becomes the new mirror (the seam `instruments.md` documents). A contract export violates VISION twice: the contract's thinness, and validation before a frame renders. A vendored real parser was priced and declined — still a second model of JS where execute-don't-mirror demands one, ~100KB into every install cache, and built ahead of evidence: every finding named here was caught by review-then-arm, none escaped to a shipped film. The seniority resolution is recorded with the disposition: "execute, don't mirror" applies where the subject is kit code with one canonical source to execute; the tables are film data whose decidability-without-running is the property `check` exists to exploit. The reader's limits are declared at its verdicts (0.16.70, 0.18.2) and in `instruments.md`; each new finding is fixed per-instance behind an arm recorded red first. **Revive trigger, cost-scaled per the Promotion table's heaviest tier ("blocked or reliably wrong"): a false verdict from this reader reaches a shipped film** — at which point the parser option is priced properly before any execution shape is reconsidered |
...
<!--due: 2026-08-24 | run BOTH same-date checkpoints and report separately: the muted-blocks count (violations over opportunities PER CLASS, minimum exposure 10 per class or that class extends; one derivation pass over the diff since 603a2ab) and the claims-reminder evaluation (corrections-per-opportunity across both windows, classifiers frozen in the script header, minimum exposure 10 per window or the checkpoint extends; delete-don't-tune only on a powered comparison). Rows above and scripts/claims-reminder.sh carry the full terms-->

| determinism-failure MAGNITUDE and location reporting — VISION's one remaining honestly-failing criterion | a pass/fail determinism check cannot tell a different film from one bit of one pixel, and cannot serve the owner's stated purpose for determinism (owner, 2026-08-04, in conversation): rebuild the same scene under new tooling and compare — better, worse, cheaper, slower — which needs "roughly the same" to be checkable, with slight difference becoming legitimate around physics. Magnitude-with-location is the instrument that expresses "roughly"; VISION carries the criterion as failing and nothing owned it until this row | the next determinism red anyone diagnoses by hand, or the first cross-tooling scene comparison (the portfolio's `gearbox` regression baseline is that comparison by design) |
| ~~align `derived-counts.js`'s HISTORICAL list with `CLAUDE.md`'s dated-record class for `snapshots/`~~ **RESOLVED BY ARCHIVAL 2026-08-07** — the owner untracked `snapshots/` entirely (moved out of the tree, gitignored), so there is no tracked surface for either classification to apply to and the conflict has no subject | the conflict fired once — a moved historical count went red in the 2026-08-04 snapshot and took a `count-mention` marker to land; archival dissolved it rather than either proposed fix | none — row kept as the record of the resolution |
...
| regenerate `site/posters/scenes.jpg` — the front-door composite is stale | the image predates `hauler` and `strider-intro` (last touched 2026-07-24, per `git log`; found 2026-08-08 by the first README audit under `audit-claims`' new routing): it shows a sampler that used to be the whole corpus. The root README's caption now says "sampler" so the prose no longer over-claims, but the honest fix is a regenerated composite covering the corpus | the next site deploy, or the next time any film's poster is shot |
| **in-scene extent validation — REP4's value half, the predecessor's foundational unshipped design (owner-raised 2026-08-08)** | REP4 closed the FORM (bare numbers warn; derived extents cannot disagree with their construction at build time), and that changed the residual gap's shape: what remains is (a) POSE-DEPENDENCE — an extent is static, geometry is animated, and corpus row 11 measures it (walker width 3.12 at t=5, 5.76 at t=45, fallen), so any single-pose check misses the calibration target's own class; (b) derivation-formula error (the rig's framing estimates are estimates); (c) named pads' values. The architecture-clean design is the predecessor's, modernized: a SUBJECTS entry optionally names its object, and the KIT at load poses the scene at each shot's `t` (pure, so free), measures the subject's world AABB, compares against what `subjectExtent` frames — under-declaration loud, over-declaration a warn. In-scene code, so no prime-directive exception and probe stays authoring-only. This is the VALIDATION variant the predecessor specified ("under-declaration throws; over-declaration warns" — the section above), distinct from the DECLINED constructor helper; a false comment once claimed it existed in smoke. Cost is a SOLVER/kernel fence change across every carrier plus the cascade: its own session, red-first on the corpus fixture, which already carries the characterized wrong extent | the corpus extent-migration sweep (its per-film probe cost is this check done by hand, once, without the standing benefit); OR Phase 4's first bake (baked motion makes framing correctness load-bearing); OR the first cold build that ships a payoff crop |
| **a disclosure audit at session close (owner-raised 2026-08-08), designed, not yet built** | the measured basis is the prelude/lookup finding (scene-analyses, three builds: every reference read landed in a 1.1/3.5/6.6-minute opening window against 178/142/108-minute sessions), so disclosure rides three channels only — SKILL.md (the one guaranteed read), tool output on the success path, and failure/warn text — and the audit has four parts: (1) MECHANICAL unreachable-reference check: every `references/*.md` is named in SKILL.md or in a shipped tool's printed strings, derived as a set comparison, red on an unrouted new file; (2) MECHANICAL verb coverage: every `build.js`/`smoke.js` verb and flag appears in SKILL.md, derived from the dispatch; (3) JUDGMENT per diff: each added capability names its disclosure channel and moment — "in a reference body" is a finding; (4) the can't-do surface lives in SKILL.md as reformulations ("no physics: bake unbuilt — author closed forms"), held honest by `audit-claims`' existing SKILL.md routing plus a named cascade step. Both mechanical halves go through `controls.md`'s door with red-first brackets; the empirical measure stays the cold-build analysis (the descent metric), which no static check replaces | the next warm session that finalizes plugin-content changes builds it; the cold build one tier down measures whether the situated-pointer instances (0.26.0's FAIL pointer, 0.28.0's teaching warn) changed behavior |
...
| ~~a static check that no `SUBJECTS[*].pos` body calls `getWorldPosition`~~ **BUILT 2026-08-07, same day as filed** — `selfcheck.js` check 23, with two arms in `bracket-selfcheck.js`. Red observed on real impure code (exit 1) and green on the fixed corpus (exit 0). Its first draft fired on the two scenes that had just been FIXED, because their comments *name* the impure form while warning against it — a check that cannot tell an instruction from a warning about that instruction reports the documentation as the defect. Comments are stripped before the test now, and the second arm pins exactly that: a SUBJECTS block that only mentions `getWorldPosition` in a comment must stay green. Disclosed proxy limit, in the check's own header: it is a text scan, so it sees a direct call and not one reached through a helper — it can reject, it cannot approve | `pos` is specified as a pure function of `t`, and the obvious way to frame a rig's part violates it: `getWorldPosition()` reads a matrix written by the PREVIOUS `animate()`, because `seekTo` runs `setCamera` before `animate`. **Two corpus films were written this way on consecutive days** — `strider-intro` shipped with it and passed smoke, `hauler` failed its first smoke at `t=16.64` — which is the shape that argues for a check rather than prose: the determinism sampler catches it only when a sampled `t` lands where the stale pose moves a byte, so a green run certifies the samples and not the property. Filed rather than built today because the natural implementation is a source scan and this session had already spent its control budget on `bracket-dof.js`; the honest note is that prose in `film-language.md` is now the only thing guarding it. Natural red arm: a fixture whose `pos` reads a world matrix must be REJECTED, and one deriving from a closed form must pass | the next `selfcheck.js` edit for any reason, or a third film written with the impure form |
| ~~`bracket-selfcheck.js`'s cascade arm reports MISSED whenever a version bump is in flight~~ **FIXED 2026-08-07** — both triggers fired at once (a fourth sighting, during an edit to the file). The arm now takes an optional precondition; when `plugin.json` is dirty it SKIPS and prints why, and the runner reports `N arm(s) exercised, M skipped` beside its derived total. Verified the skip is conditional rather than permanent — the arm goes back to CAUGHT on a clean tree — because a precondition that always fired would silently disable the arm, which is worse than the MISSED it replaced. Original terms below | the arm injects plugin content and expects check 11 to go red, but check 11 diffs against the last commit touching `plugin.json`, so an uncommitted bump already satisfies it and the injection has nothing to prove. The behaviour is documented in the arm's own comment and confirmed both ways (2026-08-02), so this is disclosed, not hidden — the cost is that the disclosure lives in the source while the surface says *"selfcheck is not catching what it claims to… Do not trust it until this is 0"*, and a cascade commit is exactly when someone runs it. **Observed three times now** — 2026-08-02, mid-batch on 2026-08-05, and 2026-08-07 — each time diagnosed rather than fixed, which is the shape of a control people learn to read past. Fix shape: the arm detects the pending-bump state and SKIPS with its reason stated, rather than scoring itself MISSED; through `controls.md`'s door, since it changes what a control reports | the next `bracket-selfcheck.js` edit for any reason, or a fourth sighting |
| the 2026-08-24 muted-blocks count, run alongside the claims-reminder evaluation (postmortem 2026-08-03, forward item 3) | the dev-conventions TDD and doc blocks were muted 2026-08-03 (`603a2ab`); violations since the mute of what those blocks used to say — unrecoverable test claims, missing freshness markers selfcheck does not already catch — decide a question in the skills repo: zero is the evidence its ambient-tier deprecation waits on, nonzero says what its scaffold must carry. Report the count either way; it must be derived, not remembered. Filed here 2026-08-04 when the next-session handoff memo — its only prior home, local and self-replacing — was retired. **Amended 2026-08-04, pre-registered before the window closes** (the skills repo's denominator memo, relayed by the owner): a bare zero is uninterpretable without exposure, so report violations over opportunities PER CLASS — TDD: test cases added in the window; doc: doc files created or edited where a marker applies and selfcheck does not already enforce it — decide per class rather than pooled, extend any class with fewer than 10 opportunities to the next checkpoint instead of deciding on it, and derive count and denominator in ONE pass over the diff range since `603a2ab`. The claims-reminder retirement measurement shares the date and stays a separate instrument with a separate consumer | the date arriving |
| ~~**a skill for cold-session transcript analysis** (owner, 2026-08-07)~~ **BUILT the same day** as `.claude/skills/analyze-build-session/`, from the retro on the three-build pass rather than from the hypothesis below. What the hypothesis got right: the extraction schema, the complement-of-references field, and model-plus-effort as mandatory. What the retro ADDED, none of it anticipated: the orchestrator/subagent split (depth per build is a different job from comparison across builds, and a subagent cannot do the second); five named query traps that each cost real time in the first run, the worst being that tool results are `user`-type messages so the naive human-turn count is ~5x high; the verify-against-the-current-tree pass, since the analysed build ran an older plugin; the verify-against-the-code pass, which caught a deliberate warning about to be written up as a false positive; and recurrence-across-independent-builds as the explicit promotion bar. What it dropped: nothing, but "mine the timeline" turned out to be the least useful instruction in it — the timeline is cheap and the complement is where the yield is | — |
| **references are read as a PRELUDE, never as a lookup — so routing from an index cannot work, and the pointer belongs in the instrument** (2026-08-07; the strongest structural finding the cold builds have produced) | Measured across the three 2026-08-07 builds `(local)`, transcript-derived: every reference read happened in an opening window of **1.1 / 3.5 / 6.6 minutes** against sessions of **178.6 / 142.0 / 107.8 minutes**. **Not one reference was opened mid-build, in any of them.** The apparent exception proves it — one build grepped `delivery.md` and `recordings.md` at 14:23, *after* delivery, and only because a user follow-up forced a citation check. Open rates over the four analysed builds: `method.md` and `bibles.md` 4/4; `characters.md`, `materials.md`, `film-language.md` 3/4 (the miss is the 2D build, where they are largely N/A); `instruments.md` 2/4; `glossary.md` and `breakdown.md` 1/4; **`webgpu-stack.md`, `delivery.md` and `recordings.md` 0/4** — while all four exported MP4s and three hit determinism failures. That is not an editorial problem with those three files, and rewriting them would not touch it. It is structural: SKILL.md's routing table sits at the bottom of a document absorbed once at activation, and asks to be consulted at a moment when nothing is being consulted. The set splits by WHEN it is needed — **prelude** (before authoring: method, bibles, characters, materials, film-language — these work as-is) versus **situated** (at a failure or decision arriving mid-build: instruments, webgpu-stack, delivery, recordings — these fail as-is, at a measured 0–50% open rate). Fix shape, and it is a HARNESS change rather than a docs change: **the instrument that creates the need carries the pointer.** smoke's cross-reload failure text names `references/webgpu-stack.md` and its section; `build.js all`'s encode warning names `recordings.md`; a check whose green cannot see something names `instruments.md`. Measured cost of not doing it: ~53 minutes across three builds hand-rebuilding what one unopened file documents. Through `controls.md`'s door — these are message changes on failure paths, so the red-first arm pins that the pointer appears *without the verdict moving* | the next `smoke.js` or `build.js` edit on a failure path, or the next build that rebuilds something a shipped reference already documents |
| **the model-and-effort FLOOR is an outcome to track, not a confound to hold fixed** (owner, 2026-08-07, correcting this session's first framing) | The analysing session wrote model/effort variation up as contamination and advised holding it fixed for a clean doc-attribution reading. Backwards, and the owner's framing replaces it: **if the plugin is getting better and agents are learning to work with it, the model tier and effort level required to ship a film should FALL over time.** That descent is the headline measure of whether any of this work lands — more informative than any single build's smoothness, and invisible if the axis is pinned. It costs nothing to collect: both fields are already in the extraction schema. What this does not license is the reverse error, which still stands: a reference-read count that rose while the model also got bigger cannot be credited to the doc work, and a floor measurement needs the SAME brief run at descending tiers, not four briefs at four tiers. Recorded so far `(local)`: 2026-08-04 sonnet-5 (no effort field in its log at all), 2026-08-07 opus-5 at xhigh, xhigh and high. No descent is demonstrable from that — one point per configuration, which is the honest state | the next cold build: run the same brief one tier down from the last configuration that shipped clean, and record whether it still ships |
| ~~the hypothesis this row was filed with, kept for the record~~ — the practice has now run on four builds and nothing makes it repeatable | the owner should not have to name the mining axes on every run, and the analyzing session should be the one reporting which axes paid and which were noise. The method currently lives in one precedent record (`docs/scene-analyses/2026-08-04_market-crash-cold.md`) plus private notes, so each run re-derives it from prose and the axes drift. **`extract-patterns` is not this and should not be stretched into it**: it reads finished artifacts and proposes reference promotions, and has no transcript, timeline, or guidance-versus-behavior axis — using it here would put two different judgments in one file, which is the failure it was built to avoid. Inputs this pass had to re-derive by hand and a skill would standardize: session rollups; **model AND effort**, which varied across builds and were recorded nowhere until queried (2026-08-04 ran sonnet-5 with no effort field in its log; the three 2026-08-07 builds ran opus-5 at xhigh, xhigh and high — queried from the build transcripts `(local)` 2026-08-07, not remembered); plugin version read from the transcript rather than assumed; the reference-read **complement** (which shipped docs were never opened — the finding with the highest yield so far); a shipped-tool verb census with exit codes; real-human-turn extraction, which needs tool results and system reminders filtered out or the count is wrong by 5x; and the builder's-own-account-versus-transcript diff. Hard constraints to bake in: thinking text is stored EMPTY, so the behavioral window is narration plus tool calls and any claim about reasoning is unavailable rather than inferable; and every output cites private artifacts **by class only**, never by path, filename or workspace name | this session's close-out retro — the skill's contents get decided from what the three-build analysis actually found useful versus wasted, not from the list above, which is a starting hypothesis and is expected to lose items |
| **BUILT 0.26.0** — `smoke.js --dump-frames` writes the two disagreeing PNGs beside the scene on either determinism failure, and a failing run without the flag names it; `templates/bracket-readback.js` pins artifacts-without-verdict-change, observed MISSED against the pre-change tree first. ~~`smoke.js` emits the two disagreeing frames when a determinism arm fails (`--dump-frames`, or unconditional on FAIL)~~ | **three independent builds reinvented the same missing flag on the same day** (`docs/scene-analyses/2026-08-07_turtle-pair-and-v2v.md`): every one of them copied `smoke.js` and patched `writeFileSync` calls into the reload check, because the arm reports THAT two frames differ and never WHAT differed. Around 53 minutes went into hand-built differs across the three — one wrote a PNG decoder in Python, one wrote a from-scratch Playwright reproduction of smoke's own comparison, one wrote a canvas-readback discriminator. The repo's own verdict-states-scope principle (`controls.md`) covers what a green means; nothing covers what a red hands you. Highest-leverage shipped-tool gap in the set, and the clearest independent-reinvention count the ledger has recorded | the next `smoke.js` edit for any reason, or a fourth build patching it. Through `controls.md`'s door — the arm's own pass/fail behaviour must not move, so the red-first bracket pins that dumping changes artifacts and not verdicts |
| **BUILT 0.26.0, in the fix's own shape** — the named cause now appears only on the branch whose readback evidence supports a scene-side defect, demoted to "one known cause among several", below the measured layer attribution; the capture-layer branch explicitly clears the scene. ~~`smoke.js`'s cross-reload failure text names a cause that was absent in all three 2026-08-07 builds~~ | the message ends *"Usual cause: a seeded or unseeded random drawn once at load rather than derived from t"*, and in all three builds there was no such random; in one it actively misdirected the hunt. That build's own postmortem states the principle better than this row can — *"A message that names a cause is more useful than one that does not, right up until the named cause is absent, at which point it costs more than silence would"* — and it is the same shape as the `strip` moving-camera caveat and `check`'s h/w/d caveat: text that reads as a diagnosis when it is a prior. Fix shape is NOT deleting the hint but demoting it below what the run actually measured, so the measured line lands last | bundled with the `--dump-frames` row above, since both edit the same failure path and a dumped frame pair is what would let the message stop guessing |
| `build.js check`'s STYLE stand-in warning fires on the shape `bibles.md` recommends | 25 impressions across two 2026-08-07 builds (10 and 15), every one on `const STYLE = BIBLES.<name>` — the documented-correct pattern, which the literal reader cannot resolve. **This is not a false positive and must not be fixed by relaxing the check**: the code comment shows it is the declared-substitution rule working as designed, pinned by `bracket-check-kit.js`, and `feedback_signal_honesty` is exactly the rule that a check must never be quieted by touching what it measures. The cost is nonetheless real and one-directional — a warning that fires on every run of every correctly-written film trains the warning-blindness the corpus doctrine warns about. Any fix keeps the declaration and removes the repetition: resolve one level of `BIBLES.<name>` indirection in the literal reader, or state the substitution once per run rather than per shot-class. Through `controls.md`'s door, red first, with the bracket pinning that the warning still fires when STYLE is genuinely unreadable | the next `build.js check` edit for any reason, or a third build reporting it |
| **a lead, not a finding: three sub-perceptual determinism reproductions against the open 1-in-6 `WEBGPU=metal` FAIL** — **first step DONE 0.26.0**: the discriminator ships inside `smoke.js`'s determinism checks (readback beside every screenshot, layer named in the verdict, `bracket-readback.js` red-first both ways), and the baseline is measured — 24/24 green on `bear-and-bees`, `WEBGPU=metal`, `playwright-core@1.61.1` (2026-08-07, one machine; a standing 1-in-6 rate survives that sample ~1.3% of the time). The lead itself stays open: the flake did not reproduce, so the discriminator has never seen it live. **The bump EXECUTED 2026-08-07 (0.27.0, owner-directed)** — 1.62.1/Chromium 151.0.7922.34 ran the identical battery: corpus green, readback/driver/determinism brackets green, 24/24 metal runs on the suspect film on both versions. Nothing moved that the baseline can see, so the hold is spent and every future measurement is on the new backend | `plan.md` and the changelog carry that FAIL as unreproduced with `bear-and-bees` as the suspect and a local prototype as the only candidate reproducer. On 2026-08-07 three builds that were not looking for it each hit a sub-perceptual cross-reload failure, all shadow-adjacent: one fixed by `receiveShadow=false` on 34,000 instances plus an octave cut, one green 6/6 on both backends with shadows off, one at 10 differing pixels of 230,400 at max delta 1 which **shipped red with the failure measured and disclosed**. The most useful single measurement is the first build's discriminator — *screenshot differs 6/8, canvas readback differs 0/8* — which points at the OBSERVATION layer rather than at the scenes, and would retire a whole suspect class if it holds. Recorded as a lead because none of it was collected as a control: three uncontrolled reproductions are a hypothesis, not a diagnosis, and the readback result in particular needs re-deriving before anything in `plan.md` moves | someone taking the metal FAIL as a task — this row is the starting evidence, and the first step is reproducing the readback discriminator as a real bracket rather than trusting a build session's throwaway script. The owner-agreed sequence (2026-08-07) — discriminator as a bracket on 1.61.1, baseline, then bump — ran to completion the same day, in that order, so the hold it imposed is spent and is no longer a rule. Why the order mattered, kept as history: Chromium 149→151 is two majors, a different ANGLE and Skia, which invariant 5 makes a backend change in all but name — bumping first would have invalidated every determinism baseline at the moment they were the measuring instrument. `three` needed no bump; 0.185.1 was already the latest published (checked 2026-08-07) |
...
- *It went too far.* The apparatus generates its own work: a large share of
  ...
  become a second standing backlog beside this file. *(No longer true as of
  2026-08-04: R5 closed and the file deleted itself. The observation was
  accurate for the span this question describes, so it stands as evidence.)*
