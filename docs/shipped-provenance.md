...
|---|---|---|---|
| `SKILL.md` | routing and workflow order | 2026-08-04 (full claim audit against 0.19.2), spot-amendments verified 2026-08-05; step 4's `strip` bound form and `band` line verified at write 2026-09-10 against `build.js`'s dispatch and `bracket-commands.js` | audited before the first cold-start build; step 7/8 additions verified against `build.js` and that build's transcript-derived record |
| `references/method.md` | the three failure axes, beats/controls discipline, review passes, determinism idioms | 2026-07-24 (full source audit); semantics/continuity additions 2026-08-04/05 verified against the cold build's record; 0.25.0 harvest additions verified at write 2026-08-07 — span form against the KERNEL fence's `ss`/`bump`/`beatAt`, pool clause against both turtle fixtures' code `(local)`, control shapes against the warm build's postmortem `(local)`; 0.29.0 additions verified at write 2026-09-10 — the record-clock snippet against the timeline film's code and its green `check`/`smoke`/`strip` run `(local)`, the caption-band bullet against that film's first contact sheet `(local)` and `smoke.js`'s caption checks (overflow and reading speed only); 0.30.0 amendments verified at write 2026-09-10 — the `band` and `strip`-bound sentences against `build.js` and green runs of `bracket-band.js` and `bracket-commands.js`; 0.31.0 caption-wrapper sentences verified at write 2026-09-10 against the 2D template and green runs of `bracket-camera2d.js` and `bracket-band.js` | dangling-pointer class known: a straight read does not test pointers, only following them does |
| `references/instruments.md` | what each check can and cannot see; measured brackets | brackets run-and-made-fallible 2026-07-29; `check` section verified 2026-08-02 and re-verified 2026-08-04 against the 0.18.2 verb; no-instrument ledger extended 2026-08-05; probe-countermeasure correction 2026-08-07, derived from a build fixture's hand-rolled-probe failure `(local)` and `build.js probe`'s one-`t`-per-invocation shape; determinism row's layer-attribution and `--dump-frames` additions 2026-08-07, verified against `smoke.js` 0.26.0 and `bracket-readback.js`'s five arms; extent-provenance row and the value/form boundary rewrite verified at write 2026-08-08 against `check`'s scan and `bracket-extents.js`'s run; `band` section and `strip` bound paragraph verified at write 2026-09-10 against `build.js` and green runs of `bracket-band.js` and `bracket-commands.js` | **never audited end to end**; many brackets measured on the predecessor and carried over — the file labels inherited numbers as inherited |
...
| `references/film-language.md` | shot vocabulary and `SHOTS[]` solver semantics | 2026-07-24 (full source audit); 0.25.0 additions derived 2026-08-07 from the SOLVER fence's own placement arithmetic (azimuth convention, distance-from-fov); extent-derivation paragraph added 2026-08-08, verified against `check`'s warn and both migrated templates | |
