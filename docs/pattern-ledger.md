...
| **declared extents rot; measured ones do not** | **9** | 3 predecessor films cropped their own payoff; 3 of 5 hand-computed extents wrong on the 2026-07-25 film `(local)`; two more on the 2026-08-07 plugin-only builds `(local)`, found by probe before a frame was rendered; the shipped base template itself (found 2026-08-08, the hour REP4's mechanism first ran): knot declared `h:3.6` against a real bounding box of 4.79, so every FS shot overflowed by a quarter | **Landed as REP4 (2026-08-08, 0.28.0)**: extents derive from construction with NAMED adjustments, `check` warns on the bare-number form (`templates/bracket-extents.js` is its control), probe stays the value-measuring instrument. `subjectFromObject` — the runtime-measuring kit helper this row once named as the fix — stays DECLINED at its fence price (owner, 2026-08-04, reaffirmed at 0.25.0); the predecessor *specified* that helper and never shipped it, while a code comment claimed it had |
...
| **per-shot camera energy** | **2** | `bear-and-bees` wanted `locked` for the hush while the film wanted `steadicam`; went all-locked. `scenes/strider-intro.html` (2026-08-07) wanted the opposite compromise — `steadicam` for the approach, `locked` for the halt and the regard — and took `steadicam` globally, because a locked approach reads as a slideshow. **Two films, two directions, same missing knob**, which is what makes this a shape rather than one film's taste | Open carry-forward. Same shape as the viewer's camera-delta seam — design them together. The second instance was produced deliberately: the film was built to reach for vocabulary the corpus had not exercised, and this is the one that does not exist |
| **built (non-DOM) text** | **3** | 2026-07-25 film: a stroke alphabet, 3 bugs, all "one letter on a wrong assumption"; both 2026-08-07 turtle builds `(local)` independently constructed letterform alphabets, forced by the same shipped constraint — `txt()` exists only in the 2D template and the DRIVER fence pins the DOM title to `BEATS[0]`, so any film wanting a title at the END must build glyphs | Deferred. Enters at the **chart tier** when it lands — a grid of 36 glyphs exposes that bug class at a glance; a title card cannot. The "second film needing built text" trigger has now fired twice over; the chart-tier disposition is unchanged, but the count is no longer the thing holding it |
...
| **screen-anchored drawing under a moving 2D camera** | **2 spellings** | `scenes/crash.html`: `applyCamera` restates a `const CAM` field by field and a `worldToScreen` feeds its screen-space causal pass (tracked copy; the cold original stays local, so whether the 2026-08-04 cold build or the 2026-08-05 revision wrote it is not established). A 2026-09-10 timeline film built outside the corpus `(local)`: a `let CAM` reassigned in `applyCamera` and a hand-written world-y-to-screen map, so lane labels stay pinned left while the camera pushes in, plus a paper gutter under them because the push slid world lines beneath the labels. **Re-derived without reading `crash.html`**; the second film was built with repo context, so it counts as an instance, not as cold-build evidence | **Landed 0.29.0** on the second instance: the 2D template's `applyCamera` restates an exported `CAM` and `worldToScreen` ships beside it, with the gutter advice in its comment. Outside every fence, so no carrier moved; `crash.html` keeps its own spelling, which is the corpus's business, not the template's |
| **caption-safe clip for a zooming 2D camera** | **1** | the v2 of the 2026-09-10 timeline film `(local)`: `band` read 6.0-8.0% of the caption pill over ink on the first version's five zoomed beats, because a hand-moved layout still left a label row in the band. The v2 clips the world pass in screen space above the band and reads 0.0% on all seven beats. The band's position was re-derived by hand from `#cap`'s CSS (bottom 5.5% of the frame, pill about 5.6 units tall), which the kit knows implicitly and exports nothing for. Not the same shape as `scenes/crash.html`'s two clips, which bound a halt banner and its stripes | **Promoted at one instance by owner call (0.31.0)**: "that won't be the only time we build a 2D zoom in/out with captions". The count rule defers when a shape's recurrence is uncertain, and here it is not: every captioned 2D film with a push-in meets it, and the first version showed it fails silently. The 2D template now carries `CAP_SAFE_Y` and `clipAboveCaption()` and its demo uses them; `bracket-camera2d.js` holds the line to the laid-out pill at two window shapes, and `bracket-band.js` shows content pushed under the pill reading 80.6% without the clip and 0% with it |
| **event clock to scene position (the inverse of a time map)** | **1** | 2026-09-10 timeline film `(local)`: a film plotting dated events from a record needed "at what `t` does the record's clock reach this event" for every appearance, and wrote a piecewise-linear key table (holds and slow motion as keys) plus its inverse by hand. `warp()` maps scene time forward only. The missing inverse also cost an instrument pass: the first `strip` over the film's key motion was aimed by hand and ended just as the motion began | **Carried as a tested snippet in `method.md` (0.29.0), at one instance, by owner call** — reference prose, not kit code, so the kit-promotion trigger still waits for a second instance. It is the authoring half of `VISION.md`'s unbuilt separation of scene position from presentation time; design a kit helper and that separation together when either triggers. A film built from a timestamped record (a log, a transcript, a ledger) is the register that will produce the second instance |
