...
| instrument output-naming friction, two datapoints | same build: `strip` writes one fixed `<name>.strip.jpg`, so consecutive strips overwrite each other's evidence (three strips ran; one survives); and the `nocap` sheet writing `<name>.nocap.sheet.jpg` cost the session one wrong-file read — the tool printed the path, but past the session's `tail` filter | a second session tripping on either |
