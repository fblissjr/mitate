# Recordings: a lossy copy, inline on GitHub

Everything here exists to serve **one constraint: GitHub will not render an mp4
inline.** That is the whole reason a compressed loop exists. Where the constraint
does not apply — any page you control — the answer is not a better recording, it
is no recording; see `delivery.md`.
