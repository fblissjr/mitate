...
| what would the representation boundary look like in practice — one argued position, with examples, tradeoffs and a roadmap | [`representation-exploration.html`](representation-exploration.html) | as an input to the representation session. **A draft arguing one side, not doctrine** — `representation.md` owns the decision and wins wherever they disagree |
