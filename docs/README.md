...
| **what should I work on next** | the immediate queue is **[`working-plan.md`](working-plan.md)'s "What the REP2 review taught about review" (2026-08-03)** — the 0.18.1 red-first cascade. Behind it, two live tracks: **[`restructure-2026-07.md`](restructure-2026-07.md)** while that migration is open (it carries a current-position block), and **[`plan.md`](plan.md)'s representation track (REP0–REP6)** — REP1 (0.17.0) and REP2 (0.18.0) are MET; REP3 and REP4 are the open, path-independent phases | at the start of a work session. Label key, because one letter means three things: `R0`–`R5` are the restructure plan's gates, `Phase R` / `Gate R` is `plan.md`'s restructuring phase, and `REP0`–`REP6` is the representation track |
