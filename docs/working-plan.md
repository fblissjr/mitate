...
> permanent record is the CHANGELOG entries (0.16.30–0.16.67) plus the closure
> record in `snapshots/2026-08-04/` (named, not linked — a live document does
> not link into a frozen one). **The live queue is [`README.md`](README.md)'s
...
> closed, PR #3 merged). `snapshots/2026-08-04/working-plan-superseded.md`
> holds them verbatim with the selection criterion. What was still true in the
...
| align `derived-counts.js`'s HISTORICAL list with `CLAUDE.md`'s dated-record class: `snapshots/` is doctrine-classed as a dated record and scanned as a live-claim surface (postmortem 2026-08-04_session_r5-closure, forward item 1) | the conflict fired once — a moved historical count went red in the 2026-08-04 snapshot and took a `count-mention` marker to land; resolving means either adding `/^snapshots\//` through controls.md's door with the bracket run both sides, or the owner ruling snapshots keep live-claim scanning and amending CLAUDE.md's "same class" sentence instead | the next `derived-counts.js` edit for any reason, or the next snapshot that needs a `count-mention` marker |
