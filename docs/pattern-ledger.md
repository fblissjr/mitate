...
| **caption-safe clip for a zooming 2D camera** | **1** | the v2 of the 2026-09-10 timeline film `(local)`: `band` read 6.0-8.0% of the caption pill over ink on the first version's five zoomed beats, because a hand-moved layout still left a label row in the band. The v2 clips the world pass in screen space above the band and reads 0.0% on all seven beats. The band's position was re-derived by hand from `#cap`'s CSS (bottom 5.5% of the frame, pill about 5.6 units tall), which the kit knows implicitly and exports nothing for. Not the same shape as `scenes/crash.html`'s two clips, which bound a halt banner and its stripes | Deferred at 1. If a second instance arrives, the template candidate is an exported caption-band rect (so a scene need not re-derive it from CSS) plus a clip helper for the world pass; `band` is already the check that would red-first it |
